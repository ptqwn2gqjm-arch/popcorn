...
function contentToHTML(text) {
  ...
  return text.split(/\n\n+/).map(p => {
    ...
    // !![alt](url) → 超寬幅圖片
    const imgMatch = trimmed.match(imgLineRegex);
    if (imgMatch) {
  ...
  return `
    <div class="card" onclick="location.href='review.html?id=${a.id}'">
      <div class="card-image">
        ${a.cover_image
          ? `<img src="${a.cover_image}" alt="${a.title}" loading="lazy" style="object-position:${a.cover_position||'center center'}" onerror="this.parentElement.innerHTML=filmPlaceholder()">`
          : filmPlaceholder()}
